...
class CreateClientDailySummaries < ActiveRecord::Migration
  def change
    create_table :client_daily_summaries_yyyymm, :id => false, :options => 'SORTKEY (insertion_order_id, report_date)' do |t|
    ...
    grant_select :client_daily_summaries_yyyymm
    
    create_view :client_daily_summaries, :partitioned => :monthly
    grant_select :client_daily_summaries
  end
end
