class AddNewTnUserIdToEvents < ActiveRecord::Migration
  def change
    ...
    add_column :impressions, :new_tn_user_id, :string, :encode => :lzo, :limit => 100, :partitioned => :weekly
    add_column :win_notices, :new_tn_user_id, :string, :encode => :lzo, :limit => 100, :partitioned => :weekly
