...
class CreateConversions < ActiveRecord::Migration
  def change
    create_table :conversions, :id => false, :options => 'DISTSTYLE KEY DISTKEY(tn_bid_id) SORTKEY (report_partition, event_time)' do |t|
      ...
      t.string :tn_udid_sha1, :encode => :runlength
      t.string :tn_udid_md5, :encode => :runlength
      t.string :tn_device_type, :encode => :bytedict
      t.string :tn_idfa, :encode => :runlength
